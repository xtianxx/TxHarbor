...
- [x] T017 Wire recovery loop + gating readers in `internal/indexer/coordinator.go` (reuse `lease.go` unchanged; R1, R6; depends on T010, T011; done when: recovery loop rides the existing RunQuatro discipline with no new lock order; startup/loop/health readers observe the recovery row the same way they read pause rows today; `lease.go` needs zero changes — asserted, not assumed)
- [x] T017a Wire recovery loop into the serve startup path in `internal/app/serve.go` + `internal/config/config.go` (B4 review finding: RunQuatroPlusRecovery/NewRecoveryLoop existed but serve.go ran RunQuatro and no task owned serve wiring; T017's scope stays coordinator.go — this is a NEW task, not a narrowing of Phase 2; depends on T010, T011, T017; done when: serve parses `TXHARBOR_REORG_MAX_DEPTH` raw + `TXHARBOR_REORG_REPLAY_BATCH` (INDEX triple reused for timing, no new knob names), invalid max-depth refuses startup via the serve fail() path before any listener/connection, the fifth stream is constructed via `indexer.NewRecoveryLoop` with ethClient/pool cleanup parity and run via `RunQuatroPlusRecovery` under the same acquisition loop/heartbeat/cancellation/error polarity, recovery state is observed via `LoadRecoveryState`/`AnnotateRecoveryHeight` without touching readiness (active recovery never flips readyz; recovery activity, chain pauses, result validity stay three separate signals), and startup-path lifecycle tests — five-loop fan-out incl. recovery via fakes, bad-depth refusal, clean cancel, loop-error propagation per runStreams rules — are green; no story task (T020+) checked off by this edit)
- [x] T018 [P] Unit tests for depth math and config refusal in `internal/indexer/reorgpolicy_test.go` (new file; V1; depends on T009; done when: exact `bound − ancestor`, closed-boundary allow/refuse, MaxInt64-tip arithmetic, missing/zero/negative/non-integer/out-of-representation refusal, no saturation/clamping value ever written to audit — all green via `make test`)
- [x] T019 Guard-matrix integration tests in `internal/indexer/reorgcommit_test.go` (new file with legal `//go:build integration` header, same shape as the 005 integration-test precedent; V2; depends on T011; done when: each 006 txn's rechecks (L/P/V/G, stale seq/phase/position fault snapshots) independently refuse with rowcounts enforced on real PostgreSQL via `make test-integration`; seq-exhaustion snapshot: events MAX preset to 9223372036854775806 (MaxInt64−1, data-model Table 1 boundary) → next `establish` refuses with zero recovery row, zero event row, zero frontier movement — no wrap, no reuse; bounded-wait behavior under injected faults uses the capped backoff from the research R5 timing table, never tight-loops)
...
- [x] T035 Audit-completeness cross-check from `reorg_recovery_events` + `deposit_observation_transitions` against every conversion in `internal/indexer/reorg_recovery_integration_test.go` runs (FR-07/21/22, V12-audit, SC-12; depends on T020, T022, T028; done when: every Orphaned conversion traceable to old source + old basis + recovery audit; every new confirmation traceable to new source + new basis; repeated Confirmed→Orphaned→Pending cycles fully preserved in the transition log)
- [x] T036 Wire CI + record the recovery resume drill in CI config and `specs/006-reorg-recovery/quickstart.md` execution log (V1–V12 run order U → I → E, resume drill; depends on T018–T019, T020–T031; done when: CI runs unit → integration (real PG) → Anvil E2E with fault injection via the pinned quickstart Environment entries (no new make target, Makefile untouched); each scenario's 100%/zero-count assertion logged pass/fail — fails are failures, not notes; this task creates the execution record, it is NOT itself a pass claim)
...
> B1/B3/B4-REVIEW DISPOSITION 2026-09-14 (read-only review of `b43bfe3`, fixed + verified in-tree, no Batch C entered): B1 CLOSED — variadic `rc ...RecoveryCapture` removed from all four commit paths (now required param), `resolveRecoveryCapture` fallback deleted, `testRecoveryCap` threaded pre-inputs at every test call site, new `reorgcapture_test.go` proves stale-capture refusal with fully matching content on all four paths (regression subset of T026; full demanded interleaving still lives in T026/Batch D). B4 CLOSED — serve startup path wires `NewRecoveryLoop` + `RunQuatroPlusRecovery` with Q1 fail()-path refusal, cleanup parity, and readiness-neutral observer; recorded as NEW task T017a (T017 scope stays coordinator.go). B3 PARTLY CLOSED — two real defects fixed in `reorg.go` assembly (FilterLogs errors now propagate instead of advancing frontiers over zero rows; unbound historical version now holds the range instead of skipping the height), covered by `reorgassembly_test.go` (fail-loud + hold + legitimate-empty-advances); Class 3 (RPC success with silently omitted logs) is a recorded capability boundary, not a defect — no multi-node/receipt reconciliation added. Ordinary-path Class-1 handling (003 shrink/fail-loud/removed-rejection) was already covered, re-verified by inspection. Batch B/Phase 2 close on these three items; T000-P stays open; T020+ stories and T034 untouched.
> BATCH-C DISPOSITION 2026-09-14 (US1–US3, T020–T024, real PG + Anvil): T020/T021 green (Anvil close-loop + crash-resume; 1-line fix: assembled log data keeps the `0x` prefix `parseDepositLogs` demands). T022 green (4 shapes + tail); T022 exposed two production defects, both fixed minimally with basis: (i) `RecanonicalizeRecoveryBlock` had no restore-only branch when the chain returns before any new-fork row replays — recovery wedged instead of reconciling (repeat flip still refuses); (ii) revive of ex-Confirmed rows violated the approved CHECK (`pending ⟺ confirmed_at/orphaned_at NULL`) — revive now NULLs both, old basis in transition snapshots + `confirm_*`, `orphan_recovery_id` retained (no DDL, no rule change). T023 green (4 bulk-orphan proofs, no prod change). T024 green (skew/start/empty); T024 flagged one further liveness wedge — below-deposit-start (or never-bootstrapped) transfers held block/log replay via the B3 hold — fixed by a deposit-scope gate in assembly (skip outside owned scope, hold only on genuine in-scope version gaps) with 2 regression tests in `reorgassembly_test.go`; Class 3 (silent RPC omission) stays a recorded non-goal. Full demanded interleaving stays in T026; T026/T034 untouched.
