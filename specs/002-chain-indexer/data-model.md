...
- 续约（同样遵守协调协议）：`BEGIN` → `SELECT … FOR UPDATE` 取协调锁（行缺失则视为无租约，
  ...
  临界区仅两条点语句。
- TTL/心跳：生产默认 ttl = 15s、心跳每 5s（≈1/3 ttl，DB 时间口径，防时钟偏斜），
  不新增环境变量；测试允许经内部参数注入较短时间。合法性由实现校验：
  ttl 与心跳必须 >0 且心跳 < ttl；租约有效性一律以 DB `now()` 判定，不得使用应用时钟。
  连接断开后不续约，租约自然过期，他实例经真实过期条件接管（接管谓词同样使用 DB 时间）。
