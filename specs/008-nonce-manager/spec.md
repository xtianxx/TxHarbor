...
- **共同契约待确定事项**：7 项保持 OPEN（见 Dependencies & Assumptions 的 OC 表），本规范不裁决；其中 3 项按范围影响度保留澄清标记（见 FR-01/FR-03/FR-04）。
- 本规范按 `docs/workflow-008-009-parallel.md`（R1 限定例外、R2 单步骤、R3 共同契约）在 008/009 并行编制窗口内单独完成 specify；不进入后续步骤。
...
008 的职责：把出站交易所需的 nonce 视为共享可变财务基础设施状态（章程原则 VII），提供并发安全的预留、与付款意图的持久绑定，以及崩溃、重启、结果未知与链视图分歧下的恢复对账。008 不签名、不广播、不持有密钥，也不创建付款意图（付款意图创建与身份为 OC-1，OPEN；本规范不裁决）。
...
- 付款意图的创建者与身份的裁决（OC-1，OPEN）；执行授权机制（OC-5，OPEN）；签名请求与交易尝试身份（OC-4，OPEN）。
...
- **D3 — 付款意图身份（缺失；OC-1 OPEN）**：绑定目标身份未定；本规范不裁决，不将任何具体方案写成既定行为。
- **D4 — sender 权威来源（缺失；OC-2 OPEN）**：隔离键的一方来源未定。
- **D5 — nonce 持久绑定的载体（缺失；OC-3 OPEN）**：绑定记录归属（008 自有 vs 复用/扩展上游）未定；表结构、锁、事务划分留给 plan。
- **D6 — 签名请求/交易尝试身份（缺失；OC-4 OPEN）**：与 009/010 的接口身份未定；本规范只锁定绑定不变量。
- **D7 — 执行授权（缺失；OC-5 OPEN）**：是否存在执行前授权及其校验方式未决；未决期间 MUST fail-closed。
- **D8 — 对账信号载体（缺失，本规范提需求、plan 定载体）**：分歧/缺口/外部消耗/结果未知的暴露方式留给 plan。
- **D9 — 可观测载体扩展（缺失，本规范提需求、plan 定载体）**：状态查询与日志字段组合留给 plan。

### Open contract items（共同契约待确定事项；本规范不裁决，状态 OPEN）
...
OC-1/OC-2/OC-3 按范围影响度在 Functional Requirements 中保留 3 个澄清标记（总数上限 3）；OC-4–OC-7 仅以 OPEN 行记录，相关条款不将其写成已定机制。

### Explicit assumptions

- 单部署单链：同一时间只服务一条配置链；多链支持不属 v1（沿用上游）。
- 一个付款意图至多一个有效 nonce 绑定；一个 nonce 至多一个付款意图主张（本规范核心不变量；付款意图身份形式未决不影响该不变量）。
...
- 同一意图的重复预留以稳定身份判定（形式见 OC-1）；MUST NOT 以不稳定运行时标识（内存地址、请求追踪 ID）作为唯一去重依据。
...
- **FR-01**: nonce 预留 MUST 以 `(chain_id, sender)` 为隔离作用域：同一作用域内非重复分配；不同链或不同 sender MUST NOT 共享或相互影响 nonce 空间。sender 的权威来源未决：[NEEDS CLARIFICATION: OC-2 — sender 从何处权威取得（部署配置 / 钱包账户注册表 / 007 请求 / 上游授权记录）？不同来源决定隔离键的取得方式与既有数据兼容性，属业务裁决]。
...
- **FR-03**: 预留与付款意图的绑定 MUST 在发起任何签名请求或广播触发之前持久化；绑定语义 MUST 至少覆盖作用域、nonce、付款意图身份、时间与状态；重复执行 MUST 幂等（原则 II/VI）。绑定的持久形态与归属未决：[NEEDS CLARIFICATION: OC-3 — 绑定由 008 自有持久记录承载，还是复用/扩展上游现有记录？决定 008 的数据归属与跨阶段读取契约]。
- **FR-04**: 每个 nonce 绑定 MUST 关联一个稳定的付款意图身份；付款意图的创建者、标识形式及其与 007 请求的关联未决：[NEEDS CLARIFICATION: OC-1 — 付款意图由哪个阶段创建、以何稳定标识存在、与 007 请求如何关联？决定绑定目标与跨阶段契约]；在闭合前，系统 MUST NOT 把任何具体身份方案写成既定行为，也 MUST NOT 依据 Accepted 状态推断付款意图存在（守卫生效范围见 FR-16）。
...
- **FR-17**: 执行授权是否存在及如何校验未决（OC-5，OPEN）；在该项闭合前，系统 MUST NOT 假定任何默认授权来源，MUST NOT 以客户端自报或请求内容为授权依据（安全规则：授权不得仅依赖客户端校验）。
- **FR-18**: 签名请求与交易尝试的身份规则未决（OC-4，OPEN）；系统 MUST 保证：同一绑定上的多次尝试可追溯至同一付款意图；MUST NOT 产生允许同一 nonce 被两个意图主张的身份或关联。
...
- **Nonce 作用域**：`(chain_id, sender)` 二元组，nonce 空间的隔离单位；sender 权威来源 OPEN（OC-2）。
- **付款意图（Payment Intent）**：绑定目标身份；创建者与标识形式 OPEN（OC-1）；本规范不创建其定义，不预设其与 007 请求的关联实现。
...
- **签名请求/交易尝试（身份 OPEN，OC-4）**：消费绑定的下游对象；本规范仅约束其不得破坏绑定不变量。
...
- 需求状态区分：【上游已确认约束】见 Background 核验结论与 Upstream Traceability；【本阶段需求】为 FR-01–FR-23 中不依赖 OPEN 项的部分；【待业务澄清】为 OC-1/OC-2/OC-3 的 3 个澄清标记（FR-04/FR-01/FR-03）；【记录为 OPEN、不在本规范裁决】为 OC-4–OC-7；【留给 plan】为表结构、锁机制、对账载体、状态命名与算法、数值表示。
- Deferred decisions（不设澄清标记，留待 clarify 或 plan）：
  ...
  - **D-03（留给 clarify/业务裁决）**：OC-1/OC-2/OC-3（已标记）；OC-4–OC-7（保持 OPEN 行）。
...
- 本规范不创建任何未来业务表与状态机实现；不宣称与 009–011 双向核对；本次交付仅为需求规范与质量检查，不进入 clarify、plan、tasks、analyze、implement。
...
| 共同契约 7 项 OPEN；008/009 并行规则；mocks 不替代集成验收 | docs/workflow-008-009-parallel.md R1/R3/R5 |

## Downstream Handoff（下游待承接契约，不创建未来业务表与状态机）

- **009（签名隔离）**：签名请求身份未决（OC-4）；签名重试必须与内容/绑定版本绑定，恢复版本变化后不得提交基于旧链视图的内容（006 FR-26）；008 提供 `(chain_id, sender, nonce, 付款意图身份, 绑定状态)` 事实，不提供密钥材料。
- **010（交易构造与广播）**：同 nonce 多尝试的允许条件由 010 设计（006 只锁定暂停边界）；结果未知先对账；同 nonce 替换保持原付款意图（本规范 FR-09）。
