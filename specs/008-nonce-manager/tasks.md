...
- [ ] T002 [P] Domain outcome/error vocabulary in `internal/nonce/errors.go` with the exact machine strings from contracts/downstream.md §1 (`allocated`, `replayed`, `allocation_conflict`, `chain_view_unavailable`, `scope_held`, `sender_not_registered`, `sender_disabled`, `authorization_invalid`, `rebuild_incomplete`, `recovery_active`, `temporarily_unavailable`, `operation_conflict`) and contracts/read-api.md §2/§3 read outcomes (`bound`, `terminal`, `not_bound`, `mismatch`, `unavailable`); + `internal/nonce/errors_test.go`. deps: T001 | FR-14/FR-17; downstream.md §1; read-api.md §2/§3 | done: unit test pins each string exactly (`go test ./internal/nonce -run TestOutcome`)
...
- [ ] T043 Deferred-acceptance record (no code): state that real 009 client integration, 010 attempt linkage and 011 intent-existence/linkage cross-checks are deferred (contracts/downstream.md §4), that 008's own acceptance is scoped to admission/reconcile/read-provider behavior (quickstart.md), and that T000-P remains open. deps: T042 | FR-18/23; downstream.md §4 | done: record present
...
- **Foundational (Phase 2)**: T004 first, then T005 (depends on T004); T006–T011 are the [P] carrier band (different files) after T001/T003; T012 depends on T006+T007+T008+T009+T010+T011; T013 on T006+T007+T008; T014 on T006+T007+T008+T009; T015 on T014; T016 on T001; T017 on T013+T016. No task carries [P] across a stated prerequisite.
...
- **Polish (Phase 8)**: T039/T040 [P] after all stories; T041 after T015; T042 after T018–T038; T043 after T042.
...
- Setup: T002 + T003
- Foundational: T006 + T007 + T008 + T009 + T010 + T011 (six distinct files)
- US1: T018 + T019
- US2: T021 + T022
- US3: T024 + T025
- US4: T027 + T028
- US5: T031 + T032 + T033 + T034 + T035 + T036 + T037 + T038 (distinct test files; each still gated by its stated prerequisite completing)
- Polish: T039 + T040
...
| FR-15 (pre-commit re-verify; OC-7 ordering) | T011, T012, T030, T033, T035 | SC-07 | V5, V8 |
| FR-16 (Accepted is not an intent/authorization) | T037 | SC-08 | V11 |
| FR-17 (authorization validation, fail-closed) | T002, T012, T037 | SC-05 | V11 |
| FR-18 (attempt/signing identities) | — (deferred; recorded by T043) | — | review + deferred |
...
| FR-22 (real concurrency/restart testing) | T018–T038, T042 | SC-01–SC-09 | V1–V13 |
...
- **008 executes its own acceptance**: the migration probes (T005), the concurrency/crash/reconcile/release/006-coexistence suites (T018–T038), and the **ReadBinding provider contract test** (T034) plus the consumer–provider lock-order test (T035) and the `=file` response-shape assertions against contracts/read-api.md. `contracts/observation.md` §3/§4 assertions (only the named hold cleared, floor monotonicity, multi-cause survival, 006 byte-identity) are also 008's.
- **009 owns its side**: the read client, retry/backoff, authorizaton re-validation and 009's own `SELECT … FOR SHARE` scope-row participation (read-api.md §4) are 009's integration acceptance and are **NOT** implemented or re-verified in this 008 task list.
