...
6. **T035 (sync)**: rebase/merge onto then-current `main` (merge-vs-rebase chosen
   ...
   Parallel-safe now: H1–H4 adaptation design + T028 test prep can proceed in this lane
   against `origin/main` (`da316d5`) read-only; **T035 sync executes FIRST
   (before any live-008/PB acceptance) — nothing requiring real 008/PB code
   goes green before the sync; T041 re-verifies everything on the merged tree.**
