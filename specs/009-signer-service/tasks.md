...
- [X] T016 [US1] Submit path `internal/signer/submit.go` (T-submit-first: insert-first → 23505 classify → row lock → gates → policy → `KeyProvider.SignTx` → persist result → `signed`; T-submit-replay incl. `outcome_not_yet_visible`). Deps: T014, T015 (tests first). Maps: FR-13/FR-14/FR-15, persistence §§1/3. Done: tests pass, `go vet` clean. **Contract-deviation batch 2026-09-17: `SubmitDeps.ScopeLock` added, scope-row FOR SHARE first in `submitFirst`, nil fails closed; re-verified (`TestSignerSubmitTakesScopeLockFirst`, `TestSignerSubmitHoldsScopeLockAgainst008Writer`, `TestSignerSubmitNilScopeLockFailsClosed` + pre-existing submit/binding/gate green, T023 green; log `/tmp/009-fix/`).**
...
- [X] T041 Post-sync migration-chain + fixture verification (test-only additions welcome; production migration set untouched): on the merged tree, verify empty-DB full chain `000001…000010` with the 9-gap fill (`WithAllowOutofOrder`), 008-era upgrade, and the compat gates (`TestLaneMigrationsExclude009` retargeted in-lane to `TestLaneMigrationsCarryReal009`; main-side exclusion restored at 009 merge); confirm PB's `internal/db/testdata` fixture hash still matches this lane's `000009` or record the re-pin. Deps: T035 + T037. Maps: R-PB8, plan Merge order. Done: chain + compat gates green on the merged tree; log retained.
- [X] T042 Final re-verification gate on the merged tree: rerun T005 (migration integration), T026 (gate reads), T028 (008 live binding), T036–T040 (H-batch), T043 (HTTP wiring + acceptance) plus `go build`/`go vet`/`gofmt`; confirm no live acceptance went green pre-sync. Deps: T028 + T036 + T037 + T038 + T039 + T040 + T041 + T043. Maps: plan Merge order. Done: all green on the contract-deviation batch tree — full `internal/signer` + `internal/db` integration ok, full `internal/app` integration ok (incl. re-gated replay + flush-failure acceptance), T023 2/2, lane guard 1/1 (WalkDir-hardened), config/metrics reused (untouched by this batch), `go build`/`go vet` (both tag sets)/`gofmt` clean; logs `/tmp/009-fix/` (fresh) + `/tmp/009-final/` (reused, validity by commit-delta); 009 merge itself is a later step, not this task.
- [X] T043 [US6] HTTP service wiring + transport acceptance (owns the R1 gap; no prior task owned it): wire `POST /signer/v1/signing-requests` (submit), `GET /signer/v1/signing-requests/{id}` (status) and the delivery bytes path per contracts/api.md §§2-4 — no invented endpoints; construct the real `*nonce.ReadProvider` + `LiveBindingReader` adapter in `internal/app/signerserve.go` and inject via existing `SubmitDeps`/`DeliveryDeps` (T023 production graph stays clean — `internal/app` importing `internal/nonce` has 008 precedent, no carve-out); replace the 501 stub; signature bytes leave ONLY through the T-deliver protected region (first response, same-id replay, every signable path), status leaks no bytes/credentials, keep re-gating/send-deadline/unknown/missing-marker semantics, no TTL grace; acceptance replays V1/V8 + auth/refusal/status/retry/delivery over real HTTP against live PG/PB/008 with at least one signed-bytes response proven to traverse the HTTP delivery protocol (not library-only). Deps: T028 + T034 + T036–T039. Maps: FR-13/FR-17, contracts/api.md §§2-4, R1/R2. Done: `TestSignerHTTPTransportAcceptance` green incl. re-gated second retry (one new delivered admission at attempt 3, byte-identical, signature unchanged) + `TestSignerHTTPWriteFailureUnknown` + `TestSignerHTTPDeliverySinkSurfacesWriteErrors` + parity; sink uses error-returning `ResponseController.Flush`, research.md success-vs-unknown wording corrected; T023 green; no credentials in logs (`/tmp/009-fix/`).
...
- **No cycles**: 009 implementation → PB-delivered (consumes, via T035); stories → Phase 2 only; T034 ← T011/T016/T026/T027 → T028; T035 ← T013; H-batch (T036–T039) ← T035; T028 ← T026/T034/T035; T040 ← T036–T039/T028; T041 ← T035/T037; T042 ← T028/T036–T041/T043; Polish → all.
...
## Contract-deviation fix batch (2026-09-17; implement, local commits only)
- Scope: marker-replay re-gate (ruling: historic marker is not a retransmit permit), submit-tx scope-row FOR SHARE, detectable HTTP flush/write failures → unknown. No DB↔network atomicity, no TTL grace, no 010/011; A-13 and T000-P stay OPEN.
- Effective new commits in this batch: reopen (`c2e8ad0`) + fix batches below (verify with `git log --oneline c2e8ad0..HEAD`); the earlier "5 commits" figure counted the pre-batch base `eff3926`.
- Evidence: fresh logs under `/tmp/009-fix/` (commands, HEAD SHAs, exit codes, per-case counts; historic log headers not rewritten); `/tmp/009-final/` reused where code is untouched (config/metrics; validity by commit-delta, see T042).
- Zero-`authorization_unverifiable` stays scoped to legal sub-paths (T040 note); the scopeless negative case is asserted independently (T021/T026/T040 + replay-gating).
...
Task count: 40 → **48** (implementation T001–T035 → T001–T043: +8 new T036–T043, T035 rewritten; PB-01–PB-05 marked externally delivered; 43 T-tasks + 5 PB = 48 definitions, zero duplicate IDs).
