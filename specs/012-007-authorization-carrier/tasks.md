...
- [x] T040 Scratch-harness for 009-file integration: source = `/tmp/pb-009ref/` copy @pinned-SHA (T002); isolation = scratch PostgreSQL per sequence, files copied into a temp migrations FS overlay — the PB lane tree never contains 009's file. Done: harness documented + green on empty sequence.
