...
Real HTTP + real PostgreSQL + real Anvil, with real 010 (and 008/009 as its dependencies): integrate 011's
real implementation + migrations into the integration workspace, then execute real joint acceptance there;
applicable joint gates complete BEFORE 011 merges to main:
...
7. Inherited 010 limited exceptions (register J4, verbatim; inputs, not re-adjudicated here):
...
provider selection (T000-P) remain OPEN. Applicable joint gates complete BEFORE 011 merges to main:
integrate 011's real implementation + migrations into the integration workspace first, then execute real
joint acceptance.
