...
- **联合验收（C10，只定义需求）**：真实 HTTP/PG/Anvil + 真实 010（经 009/008），覆盖 FR-14 场景清单；先纳入 011 真实实现与迁移到联合集成工作区，再执行真实联合验收；适用的联合门禁在 011 合并到 main 之前完成；**替身或旧机制测试 MUST NOT 被呈现为联合验证**；010 独立验收通过 ≠ 联合通过；010 自身正确性义务不整体推迟给 011。
...
**继承的 010 有限例外（register J4 原文；逐字承接，不重新裁决）**：
