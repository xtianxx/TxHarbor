...
**E1 门禁说明（双轨）**：E1 拆分为本地轨与生产轨，分别记录，避免"同一任务既阻塞又允许实现"的矛盾。
T000-L 关闭 + T020a 通过 → 可开始本地范围实现（Anvil + 可控假 RPC）；
T000-P 关闭 + T020b（本地验收）通过 → 方可谈生产接入就绪。本地验收通过不等于生产就绪，不得自动关闭 E1。

- [X] T000-L 本地实现前置检查（Anvil + 假 RPC 范围；满足即可关闭）
  - 需求：FR-12/FR-13 本地部分。依赖：无。
  - 内容：Anvil 版本固定（`ghcr.io/foundry-rs/foundry:v1.8.1`，compose 已 pin）；
    `FilterLogs` 接口确认（research R9 V1）；`config_hash` 标准向量（R9 V2）；
    本地故障注入清单完备——显式截断、达已知上限计数、缩批、单块超限、限流、超时、畸形响应（T003/T010/T011 覆盖）。
    这些结果仅证明本地行为，不证明生产 provider 行为。
  - 完成条件：上项逐项可勾选；关闭后 T001–T019、T020b 可在本地范围按依赖执行。
  - 状态（2026-09-13）：CLOSED。证据：compose.yaml pin 行已读；module cache
    `ethclient.go:473 FilterLogs` 已确认；`sha256sum` 复现 R9 V2 向量 `b4eedd…cef7f`；
    故障清单见 T003/T010/T011。用 `T000-L ✓` 标记，不代表 T000-P。
- [ ] T000-P 生产 E1（选型 + 附录 + 生产验证；保持 open）
  - 需求：FR-12/FR-13 生产部分，E1。验收场景：#4/#5 的生产等价覆盖。依赖：生产 provider 选型（未定）。
  - 完成条件：附录表逐项填完（上限/区间/标志/分页/错误码原文）+ 来源链接可打开 + 上限经实测复核；
    在此之前生产 provider 不得标为可用，`LOG_BATCH_BLOCKS` 生产值不得宣称已复核。
    本任务 open 不阻塞本地实现；门禁约束的是生产接入与部署。
...
- [X] T000 [E1-GATE] 确认 provider 文档、限制、截断与分页语义，填写 `quickstart.md` E1 附录
...
- [X] T001 [P] 新增迁移 `migrations/000003_event_indexing.sql`（3 表 + 约束 + 索引）及迁移测试
...
- [X] T002 [P] 配置：`TXHARBOR_LOG_START_HEIGHT` / `TXHARBOR_LOG_CONTRACTS` / `TXHARBOR_LOG_BATCH_BLOCKS` 解析、校验与 `config_hash`（`internal/config/`）
  - 需求：FR-04/FR-05，research R3/R6。验收场景：#10。依赖：无。
  - 完成条件：单元测试绿——小写归一化/排序去重/空白拒绝；`config_hash` 与澄清 A1 向量一致
    （`"erc20-transfer:v1\n"` 前缀、无 BOM、无尾换行；标准向量见 research R9 V2，实现必须复现）；大小写/顺序/重复差异收敛同一身份；
    批次/超时参数不在身份内；非法值拒绝启动且 `Summary` 脱敏。
- [X] T003 [P] `eth.FilterLogs` 封装 + `KindIncomplete` 错误分类（`internal/eth/client.go`）
...
- [X] T004 [P] `log_*` 指标组与观测契约测试（`internal/metrics/metrics.go`，`contracts/observability.md`）
...
- [X] T005 [US1] 区间上界与覆盖验证：`min(请求末端, 002 checkpoint)` + `[a,b]` 连续 canonical 验证（`internal/indexer/` 日志扫描文件）
  - 需求：FR-02/FR-03。验收场景：#1/#9。依赖：T001–T004。
  - 完成条件：上界越界/缺失/非 canonical/链暂停时拒绝越区推进；起点高于覆盖时等待不推进；单测 + 集成断言绿。
- [X] T006 [US1] 扫描循环 + 区间原子提交（写入 + `next_block=b+1` 同事务，精确守卫）
  - 需求：FR-02/FR-11，data-model 写事务协议。验收场景：#1。依赖：T005。
  - 完成条件：成功区间推进恰为 `b+1`（含合法空区间）；任何失败无部分提交；首区间原子建行。
- [X] T007 [P] [US1] Anvil 集成测试：混合区间 + 002 已更高后补扫（`internal/indexer/logscan_integration_test.go`）
...
- [X] T008 [US2] 幂等重放 + 冲突内容比对 + 行数核对实现与测试
...
- [X] T009 [US3] 崩溃与不确定提交恢复测试（查询后退出 / 事务中失败 / 提交响应丢失后重启）
  - 需求：FR-11/FR-16。验收场景：#3（SC-03）。依赖：T006。
  - 完成条件：重启后从持久化进度继续，无漏扫无部分提交；未知结果先重读 DB 再决策（有专用断言）。
- [X] T010 [US3] 退避/缩批/单块停推实现与测试（`KindIncomplete` 路径 + 有界退避）
...
- [X] T011 [US4] 8 项逐条校验实现 + 6 类非法日志测试（`detail.class` 七分类）
...
- [X] T016 [US6] 迁移三条件 + 约束完好测试（并入 T001 实施，本条为验收复核位可跳过——保留编号防漏项）
  - 需求：FR-10/FR-11。验收场景：#11（SC-10）。依赖：T001。
  - 完成条件：同 T001 三条件绿；本条在 T001 完成时同步勾选。
- [X] T017 [US6] `serve` + coordinator 接线：单获取循环/单心跳下双 serveLoop 并发 + 状态镜像 + 安全退出
  - 需求：FR-16/FR-17，research R1/R7。验收场景：#3（退出部分）/SC-11。依赖：T004，T006，T014。
  - 完成条件：`internal/indexer/` 新增 coordinator（唯一 Acquire 循环 + 唯一 Heartbeat，并发跑双 loop，
    任一失权/心跳丢失即全停重取）；`Scanner.serve` 机械拆分出 `serveLoop`（002 行为不变，回归测试全绿）；
    `serve.go` 改走 coordinator（双错误通道 + 双状态镜像）；`log_*` 指标随 `metricTicker` 镜像；
    coordinator 专项集成测试（失权全停、重取恢复）绿；
    终止信号下未提交零残留；配置拒绝/失权/不可重试错误按类别退出或停写。
- [X] T018 [US6] 可观察端到端：滞后/状态/暂停查询 + 脱敏审计
...
- [X] T019 全量验证：`make build` + `make lint` + `go test ./...` + `go test -tags integration ./...` 全绿
  - 依赖：T007–T015，T017–T018。完成条件：四命令一次全绿；失败按回归处理，不弱化断言。
- [X] T020a 实现前规划复核（静态一致性，不依赖任何实现与测试结果）
  - 依赖：无（仅依赖 spec/plan/research/data-model/contracts/quickstart/tasks 文档）。
    显式不依赖 T019——本任务在实现开始前即可关闭；关闭是本地实现准入条件之一（另一条件为 T000-L）。
  - 完成条件：逐项核对 FR-01–19、SC-01–11、11 验收场景在 tasks 的覆盖（见下表）；
    确认无循环依赖、无超出规格的设计；输出实现准入结论（通过 / 附条件通过）。
    本任务只放行"开始本地实现"，不等同实现验收，更不等同生产就绪。
  - 状态（2026-09-13）：CLOSED，结论通过。证据：覆盖矩阵 19 FR + 11 SC + 11 场景全映射；
    依赖图无环（T020a 零依赖；仅 T020b→T019）；设计无超规格项（3 表/3 配置项/零新端点）；
    `git diff d00f5a0 -- spec.md checklists/` 为空；R1 共享缺陷已修正为 coordinator 方案。
- [X] T020b 本地范围验收（实现后；名称固定，不得与生产就绪混称）
  - 依赖：T019（需全部本地测试证据）。完成条件：凭 Anvil/假 RPC 测试证据逐项验收 11 场景与 SC；
    输出本地验收结论 + 未解决项（T000-P open）；发现语义冲突先澄清，不私改规格。
    通过仅表示"003 本地范围验收通过"，"生产接入就绪"需 T000-P 关闭后另行判定。
  - 状态（2026-09-13）：CLOSED，结论"003 本地范围验收通过"。证据：`make build`/`make lint`/
    `go test -count=1 ./...`（150 passed）/`make test-integration`（8 包全绿，含 12 个 LogScan
    集成场景 + 双真 worker + 暂停并发；TwoWorkers 测试设计脆弱性已加固）全绿；oracle 协议复查
    1 High + 1 Medium 已修复并复检；research 签名笔误已修正；T000-P 保持 open。

---

## Dependencies & Execution Order

- **Setup (T000-L/T000-P)**: T000-L 关闭后 T001–T019、T020b 本地工作可推进；T000-P open 约束生产接入与部署，不阻塞本地实现。
...
- **Polish (T019–T020b)**: T019 依赖所有实现任务；T020a 独立于实现（可先行关闭）；T020b 依赖 T019。
...
| FR-12/13 | T003，T010，T000-L（本地）/T000-P（生产） | #4，#5 |
| FR-14/15 | T013 | #7 |
| FR-16 | T009，T014，T015，T017 | #3，#8 |
| FR-17 | T004，T017，T018 | SC-11 |
| FR-18/19 | T020a（实现前范围门禁）+ T020b（实现后验收） | — |
...
- T000-P 未关闭前，任何"生产可用/生产就绪"表述不得出现；残余风险如实声明；
  不硬编码未经确认的生产上限；禁用计数判定不等于完整性已解决。
- 验收命名固定："003 本地范围验收" vs "生产接入就绪"，不得混称。
