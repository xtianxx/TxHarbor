...
- **Decision（lease 共存）**: 同一进程内的 header scanner 与 log scanner **共享同一个 `Lease` 句柄**（同一 `owner_id`、同一心跳循环；`serve` 只创建一次 `NewLease` + 一次 `Heartbeat`，两个 scanner 共用）。跨进程仍是单主：同一时刻只有一个进程的 `Acquire` 胜出。
- **Rationale（两层机制，不存在"一方持续占用"）**: 须区分 **lease 所有权**（长持：CAS 获取 + 5s 心跳续约 + 15s TTL，决定"谁有权写"）与 **协调行锁**（短持：写事务内 `FOR UPDATE` 到 `COMMIT` 的毫秒级临界区，决定"写操作串行顺序"）。持有所有权的进程在事务之外**不占用**协调行：行锁只在短事务内存在，header 推进与日志提交在此排队，均为毫秒级，不存在一方饿死另一方。未获所有权的进程（无论 header 还是 log scanner）写事务在 lease 归属裁决即被拒绝，原地待命重试，不产生部分写入。002 的 `commitPause` 已走同一协议（`ensure → 加锁 → 复核 → 写入`，见 `scanner.go:743`），故链级暂停写入与日志提交天然互斥，不存在竞争窗口——"无窗口"的依据是行锁互斥 + 持锁后新快照裁决（与 002 data-model 并发论证相同的两条成立语义），不是"003 锁住了行"这一句话。
...
## R5 — Provider 上限与 E1：未选定，列为实现及接入的前置门禁（不阻塞任务拆解，不虚构映射）

- **Decision**: 生产 RPC provider 尚未选定（仓库唯一配置指向本地 Anvil `127.0.0.1:8545`）。因此：
  - E1 是**实现及生产接入**的前置门禁，**不是任务拆解的前置条件**：tasks 可先生成，E1 相关任务显式标注依赖，
    生产 enablement 任务在 E1 关闭前不得标记完成。
