...
Verification-round addendum (2026-09-17): gate-expiry evaluation pinned to `clock_timestamp()` (send-gate matrix; 009 `submitClockSQL` precedent); `TXHARBOR_TX_SEND_TIMEOUT` default fixed at 15s (technical). Adjudication addendum (2026-09-17): G-010-1 closed as an explicit limited Q3 exception (natural-expiry residual only; recorded + reconciled, not called in-flight; no grace/TTL; prior checks never reused). Adjudication addendum 2 (2026-09-17): G-010-2 class (c) closed as a second explicit limited Q3 exception (unperceived lock-loss window only; same-session probe mitigation; freeze-plus-manual-review on proof; reconcile never permits resend); classes (a)/(b) stand as specified. Status split: plan documents delivered; design closed except G-010-2 classes (a)/(b) residuals, G-010-4/5/6 (reported, unverified), G-010-3 FK closure specified for the joint integration workspace (000013, applicable joint gates complete before 011 merges to main); joint acceptance not executed (gated on 011's real implementation + migrations being integrated); no auto-entry into tasks.
...
Two required timelines: **(a) invalidation before the region** → observed under the held locks; zero dispatch; refusal class + observed basis committed. **(b) region first, invalidation during dispatch** → the invalidation writer blocks on the conflicting lock until the region commits; the dispatch is legally in-flight (it entered the uncancellable stage while every gate was valid), and the invalidation governs later replays/replacements only. Time-based expiry is re-evaluated with `clock_timestamp()` immediately before dispatch and the evaluated basis is recorded; its residual is the adjudicated G-010-1 exception. DB/network independent failures are G-010-2 in three classes: protection-held-then-commit-failed → send-`unknown` + reconcile; loss detected before dispatch entry → `region_aborted_no_dispatch` with business effect still `unknown` pending reconcile; unperceived lock loss before dispatch → new residual pending adjudication (never a fabricated success/failure, never a rewritten known result).
...
- **Strictly dependent (do not start before their inputs land)**: send region (needs gate read sequence + migration), reconcile (needs chain read extension), confirmation/reorg (needs canonical view reads + V8/V9 chain scenarios), joint J-matrix (needs 011's real implementation + migrations integrated into the integration workspace; applicable joint gates complete before 011 merges to main; upstream sync precedes dependent acceptance).
...
| G-010-3 | No FK to `payment_intents` until 011 lands; joint-integration closure item (integration workspace, before 011 merges to main) | 010→011 joint integration |
