...
- **G-010-2 (database-region failure vs network send).** Three classes, different handling:
  ...
  - (c) Loss unperceived before dispatch (session dies silently after step 9; revocation/pause/claim-change commits while the sender still believes it is protected; network send starts afterwards): REAL irreducible window — no mechanism lets the sender prove at dispatch time that its locks are still held (any proof round-trip is itself stale). Mitigations (not closure): a `SELECT 1` liveness probe immediately before dispatch, exactly one dispatch attempt per region, minimal entry latency, and best-effort post-hoc detection (version comparison at reconcile; proven-stale basis escalates to operator review, never auto-resends, never relabeled). This window MUST NOT be authorized by unknown-recording, hash probing, the 15s bound, "no TTL", G-010-1, or 009 precedents. **New residual pending adjudication; see ruling question.**
