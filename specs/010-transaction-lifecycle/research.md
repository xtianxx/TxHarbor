...
- **Joint (J-matrix, design requirements only)**: real 007 HTTP + 011 executor + 010 + 009 + PG + Anvil; covers intent creation, authorization supply, expired-worker fencing, joint unknown reconciliation and reorg revision (FR-16). Joint acceptance is defined here and executed only after 011's real implementation and migrations are integrated into the integration workspace (010→011 merge order; applicable joint gates complete BEFORE 011 merges to main; upstream sync before dependent acceptance). Mocks/old-mechanism tests never count as joint evidence; 010-independent success is not claimed as joint success.
...
- **G-010-1 (time-based gate changes between the last evaluation and dispatch entry; 2026-09-17 approved limited exception).** Locks order *write-based* invalidations (revocation, pause, claim re-lease, binding change) against the send region, but **expiry is a time boundary with no writer to lock**. The design re-evaluates authorization and claim expiry with `clock_timestamp()` as the last step before dispatch and records the observed basis in the send row. A natural expiry taking effect in the residual interval is recorded as a "post-final-check natural-expiry send residual" and reconciled — explicitly NOT described as legally in-flight before invalidation, and NOT extended to revocation, pause, version replacement, DB/connection failure, or any grace/TTL. Prior checks MUST NOT be reused across queueing/backoff/reconnect/retry/resume; those paths re-evaluate gates. Definitive results are kept; only indeterminate outcomes become `unknown`. **Adjudicated 2026-09-17; closure covers this residual only.**
- **G-010-2 (database-region failure vs network send).** Three classes, different handling:
  - (a) Protection verifiably held at dispatch entry (locks acquired, gates passed, session live): legal in-flight per Q3. If the region then fails to commit, the send may have happened with no committed record → send-`unknown` + reconcile by `tx_hash`; never infer "not sent".
  - (b) Loss reliably preventable (lock wait/deadlock at steps 2–9; connection loss detected before dispatch is invoked): zero dispatch is certain in-process → record `region_aborted_no_dispatch` (not a send result); the attempt's business effect stays `unknown` pending reconcile; committed refusals and definitive verdicts are never rewritten.
  - (c) Loss unperceived before dispatch (session dies silently after step 9; revocation/pause/claim-change commits while the sender still believes it is protected; network send starts afterwards): REAL irreducible window — no mechanism lets the sender prove at dispatch time that its locks are still held (any proof round-trip is itself stale). 2026-09-17 adjudicated as a second explicit limited Q3 exception: the probe MUST use the same session/transaction holding the protection locks (never a reconnect or fresh pool connection) and is mitigation only; any detected transaction/connection/protection failure MUST block still-cancellable sends; single send only with no transparent retries bypassing re-evaluation (queue/reconnect/backoff/retries re-acquire protection and re-verify all gates); minimal entry latency is an optimization goal, not a proven bound — the window MUST NOT be described as fixed, tiny, or rare by default. Reconcile compares recorded authorization/recovery/execution versions plus change evidence; a successful probe or post-hoc version equality MUST NOT prove safety at send time; unresolvable ordering stays indeterminate. On confirmed protection loss with evidence of (or inability to exclude) a post-invalidation send: freeze that intent's further sends and transfer to manual review (chain observation/query/reconcile remain allowed); normal takeover/version updates are not violation evidence. Manual review lifts only this residual's independent freeze cause (controlled permission + evidence + audit), never overrides revoke/expiry/pause/eligibility gates; full re-verification precedes any resend; reconcile never directly permits resend. Known results kept; only indeterminate outcomes become `unknown`; no second intent; no nonce release; unobservable session faults MUST NOT be claimed always-detectable. This exception covers this residual only — not actual verification, not other residuals.
