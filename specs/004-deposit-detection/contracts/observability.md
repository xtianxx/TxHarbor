...
| txharbor_deposit_pause_total | Counter | chain | 充值暂停次数（只增，与 pause 行数无关）。累积合并（revision+1）仅经诊断SQL可见：活行detail全文（首段+全部+merged段）与deposit_pause_audit(action='merge')；不新增暂停日志行，不计txharbor_deposit_pause_total；deposit_pause.height/kind列仅为首原因，次因以detail段与merge审计为准。 |
...
- 暂停：`chain_id, pause_id, revision, height, kind(upstream_gap|chain_view_changed|validation_failed), detail`（error；`detail` 含 `version=<seq>` 明确归属，不含凭据与原始数据；`validation_failed` 的 `detail.class` 取值 `bad_address|bad_topics|bad_amount|out_of_range|missing_field|identity_conflict`；需 006 时含 `needs_006=true`）。累积合并（revision+1）仅经诊断SQL可见：活行detail全文（首段+全部+merged段）与deposit_pause_audit(action='merge')；不新增暂停日志行，不计txharbor_deposit_pause_total；deposit_pause.height/kind列仅为首原因，次因以detail段与merge审计为准。
...
-- 充值暂停/停止（行存在 = 暂停有效）
-- 累积合并（revision+1）仅经诊断SQL可见：活行detail全文（首段+全部+merged段）与deposit_pause_audit(action='merge')；不新增暂停日志行，不计txharbor_deposit_pause_total；deposit_pause.height/kind列仅为首原因，次因以detail段与merge审计为准。
