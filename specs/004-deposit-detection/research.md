...
- **Decision**: 授权转换是唯一的合法身份更新路径（默认拒绝不变），语义如下：
  ...
  - 授权路径暂停写限定：只允许原样保留（不动行）或对显式匹配目标的条件 DELETE（同事务审计行）；
    禁 UPDATE／合并。累积合并（新原因段追加＋revision+1＋merge 审计行）专属暂停写事务，
    须持 lease 锁并走 Table 3 原子条件（含 Q8 的基一致与新证据锁内重验），授权事务禁用此写法。
  ...
  - 多次切换（回放未完成时再次请求）：**无需新增业务限制**。授权每次从当前持久化状态重算：
    ...
    暂停行按**累积原因**合并（2026-09-13 Q8：保留 `pause_id` 与首原因，新有效原因以
    `+merged[rev=<R>] kind=<K> height=<H> version=<S> :: <原样证据>` 段追加、`revision` +1，
    各原因类型/范围/来源版本/证据全保留且不覆盖；同一有效原因幂等零写；lease＋捕获基一致＋
    新证据锁内重验缺一即放弃，旧基证据不合入；merge 审计同事务，operator 固定且 reason 携带
    持锁 owner 实值与触发路径；授权事务禁用此写法）；
