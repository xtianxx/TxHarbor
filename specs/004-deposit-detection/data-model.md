...
| request_id | TEXT | NULL（仅首版本行）；非空时本链唯一（UNIQUE (chain_id, request_id)；首版本单行另以 partial unique 约束） | 授权请求身份（调用方提供，同链稳定；同 ID 同参认重试，同 ID 异参拒绝，异 ID 独立校验） |
...
- 请求身份规则（禁仅比目标哈希）：授权参数含预期旧版本 seq（`expected_old_seq`），仅 old_hash 不足。
  ...
  即使当前已进入更晚版本；
  命中但意图参数任一不同 → 同 ID 异参，明确拒绝，不产生状态变化；
  ...
  不得自动改为处置新暂停；同 ID 更改目标即同 ID 异参，明确拒绝。
...
1. 事务外：先做完整性分流，再按 (chain_id, request_id) 查 history 定性（禁仅比目标哈希；禁先重算派生值再比对）：
   ...
   命中但意图参数任一不同 → 同 ID 异参，明确拒绝，不产生状态变化，不开事务；
   未命中 → 独立请求继续下述流程（不同 request_id 即使意图相同也不合并）。
   ...
   此时无显式目标 → 拒绝并要求调用方读取实例后重新明确授权（重新授权须用新 request_id；
   沿用旧 ID 仅加目标按同 ID 异参拒绝）。
   ...
   （同 request_id 同参重试仍按"未命中"走独立请求，因前次未记录），
...
| 授权身份/参数不符（过期、空授权、同ID异参） | 授权拒绝 | 不产生状态变化；过期拒绝报告预期与当前版本；同 ID 异参明确拒绝 |
