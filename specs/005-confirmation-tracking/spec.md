...
- 阈值变更语义已裁决（见 Clarifications 与 FR-03）；切换、授权及并发控制的具体机制留给 plan，优先复用现有能力。
...
- Q: 缺失、非整数、小于 1 或超出系统支持整数范围的确认阈值应如何处理（启动与变更时）？ → A（选项 A，必填无默认值，拒绝不静默纠正）：确认阈值必填，无默认值。缺失、非整数、小于 1 或超出系统支持整数范围时，启动必须失败并返回明确配置错误；不得静默修正、截断或回退默认值。不额外设置业务上限，具体表示范围及安全运算方式在 plan 中明确。如果后续裁决允许阈值变更，非法变更必须被拒绝，原有效策略保持不变；本决定不预先授权运行时变更。
- Q: 确认阈值是否允许变更，生效规则是什么？ → A（选项 A，受控授权变更，与版本化配置契约一致）：阈值只能通过受控、授权、可审计的程序变更；重启携带不同阈值同样属于策略变更，未经授权的配置漂移必须拒绝。同一部署所管理的链在任一时刻只有一个有效确认策略版本；切换后旧版本计算结果不得提交，必须基于当前有效策略和一致链视图重新判定。新阈值从切换点起适用于所有尚未确认的 Pending（含此前已存在的观察）；降低阈值不直接批量确认，仍须满足 canonical、链视图有效、未暂停等完整条件。提高或降低阈值均不撤销、降级或改写已有 Confirmed，保留首次确认时间及当时的阈值、策略版本和确认依据；006 因重组使确认失效的职责不变。变更失败不得留下部分生效或多个有效版本；具体切换、授权及并发控制机制留给 plan，优先复用现有能力，不扩展为通用配置平台。
...
4. **Given** 已授权切换新阈值（提高或降低），**When** 检查覆盖切换点前已存在的 Pending，**Then** 按新阈值重新判定：降低阈值不直接批量确认，仍须满足 canonical 归属、链视图有效、未暂停等完整条件；已有 Confirmed 保持不变，可追溯当时阈值与依据。
...
5. **Given** 重启携带不同阈值且未经授权（配置漂移），**When** 启动，**Then** 拒绝沿用并报错，原有效策略与已有观察不受损；切换期间旧版本计算的确认结果不得提交。
...
- 非法阈值（缺失、非整数、小于 1、超出整数范围）：启动 MUST 失败并返回明确配置错误；MUST NOT 静默修正、截断或回退默认值（2026-09-14 澄清）。
- 阈值变更：未经授权的变更（含重启携带不同阈值）MUST 被拒绝；授权切换仅从切换点起对尚未确认的 Pending 生效，不追溯改写 Confirmed；切换失败 MUST NOT 留下部分生效或多个有效版本（2026-09-14 澄清）。
...
- **FR-03**：确认阈值 N MUST 满足 N≥1；只有属于当前 canonical 链的充值达到阈值才可确认。阈值 MUST 必填、无默认值：缺失、非整数、小于 1 或超出系统支持整数范围时，启动 MUST 失败并返回明确配置错误；MUST NOT 静默修正、截断或回退默认值。不设业务上限；具体表示范围及安全运算方式留给 plan。阈值 MUST 只能通过受控、授权、可审计的程序变更（2026-09-14 澄清，选项 A）：重启携带不同阈值同样属于策略变更，未经授权的配置漂移 MUST 被拒绝；非法变更 MUST 被拒绝，原有效策略保持不变。同一部署所管理的链在任一时刻 MUST 只有一个有效确认策略版本；切换后旧版本计算结果 MUST NOT 提交，MUST 基于当前有效策略和一致链视图重新判定。新阈值从切换点起适用于所有尚未确认的 Pending（含切换点前已存在的观察）；降低阈值 MUST NOT 直接批量确认，仍须满足 canonical 归属、链视图有效、未暂停等完整条件。提高或降低阈值均 MUST NOT 撤销、降级或改写已有 Confirmed；MUST 保留首次确认时间及当时的阈值、策略版本和确认依据；006 因重组使确认失效的职责不变。变更失败 MUST NOT 留下部分生效或多个有效版本。具体切换、授权及并发控制机制留给 plan，优先复用现有能力（004 版本化配置与授权事务先例），MUST NOT 扩展为通用配置平台。
...
- **SC-10**：授权切换阈值后，切换点前已存在的 Pending 100% 按新阈值重新判定（降低阈值无直接批量确认），已有 Confirmed 100% 不变且可追溯当时阈值与依据；未经授权的阈值漂移 100% 被拒绝；切换失败 100% 无部分生效、无多个有效版本。
