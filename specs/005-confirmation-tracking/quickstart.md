...
- 正常滞后 vs 异常：tip 可信但高度未覆盖 → 等待（state=1）；tip 缺失/暂停/不可信 → 停止（state=3）；
  单行引用不可信 → 跳过计数（`skipped_total{noncanonical}`，error 日志），循环不 halt，同批其他行照常处理。
- 前排滞留不饿死：播种前排永久不可信行（DB 直插伪造哈希，故障注入）+ 后排达标 Pending →
  同批内后排照常确认，前排仅计数跳过；持续多 tick 无饿死（单调性论证见 data-model §候选分类）。
