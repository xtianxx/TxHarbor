...
// migration_integration_test.go owns spec tasks T004/T005 for 009-signer-service:
// it proves migration 000009 applies and rolls back cleanly over a real 007
// database, that its named constraints carry the exact declared names, that
// historical migrations stay untouched (000001-000009 allowlist), and that no
// TTL/grace column exists. Every database assertion here is a raw-SQL probe;
// no signer business code is exercised. Each test boots its own isolated
// scratch PostgreSQL container via testcontainers and terminates it in
// t.Cleanup.
package signer
...
// empty), git status under migrations/ mentions only 000009, version 9 carries
// the declared name, and no embedded version exceeds the allowlist.
func TestSignerMigrationHistoryUntouched(t *testing.T) {
	...
	for v := range seen {
		if v > signerMigrationCurrentVersion {
			t.Fatalf("embedded migration version %d exceeds the 000009 allowlist", v)
		}
	}
	if len(frozen) == 0 {
...
// on an isolated scratch database: a real 007 database gains ONLY 000009, the
// status reports 9/clean, 000009 Down drops exactly the six 009 tables while
// the upstream 002 row survives, and re-up reproduces the same state.
func TestSignerMigrationUpgradeDowngradeFrom007(t *testing.T) {
	...
	// Full embedded set applies ONLY 000009.
	...
	if !strings.Contains(out.String(), "applied=1 skipped=7 pending=0") {
		t.Fatalf("upgrade output = %q, want applied=1 skipped=7 pending=0", out.String())
	}
	...
	if !strings.Contains(out.String(), "current_version=9") || !strings.Contains(out.String(), "pending=none") {
		t.Fatalf("status after upgrade = %q, want current_version=9 and pending=none", out.String())
	}
	...
	// 000009 Down via a provider mirroring internal/db: exactly version 9.
	...
	if len(results) != 1 || results[0].Source.Version != signerMigrationCurrentVersion {
		t.Fatalf("DownTo(%d) rolled back %d migration(s), want exactly version %d",
			signerMigrationBaselineVersion, len(results), signerMigrationCurrentVersion)
	}
	for _, rel := range signerMigrationTables {
		if signerMigrationRelationExists(t, sqlDB, rel) {
	...
	if !strings.Contains(out.String(), "applied=1 skipped=7 pending=0") {
		t.Fatalf("re-up output = %q, want applied=1 skipped=7 pending=0", out.String())
	}
	out.Reset()
	if err := db.MigrateStatus(ctx, fullOpts, &out); err != nil {
	...
	if !strings.Contains(out.String(), "current_version=9") || !strings.Contains(out.String(), "pending=none") {
		t.Fatalf("status after re-up = %q, want current_version=9 and pending=none", out.String())
	}
	for _, rel := range signerMigrationTables {
		if !signerMigrationRelationExists(t, sqlDB, rel) {
