...
- **J6 迁移/共享入口/配置/测试资源归属、同步顺序和联合验收责任**：迁移暂定 010 `000011_*`、
  ...
  J1–J4 真实发送验收因此尚未执行（不算通过）；集成工作区先并入 011 真实实现 + 迁移
  （`000012` 建表）使适配器读到真实 claim 行，随后才执行真实联合验收；适用的联合门禁
  在 011 合入 main 之前完成（010→011 合并顺序不变）；替身永不代替真实接线。无循环依赖：
  010 不需要 011 表即可迁移、启动与独立验收；011 不需要 010 表即可迁移与独立验收。
