...
| 租约过期/一进程退出接管 | SIGSTOP 冻结 B 后在途领取稳定，SIGKILL 模拟崩溃退出；B 持有的 pending 行在租约红线（`claim_expires_at − 1s`，见下）前保持 owner=B，其后由 A 重领（`attempt_count +1`）并发布 | `phase=crash-kill ...`、每行 `takeover row=... pre_attempt/post_attempt/expires`、`lease-takeover verified` |
